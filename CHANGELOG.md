...
- Cross-observer identity reconciliation (Part 4, §15.2): `proposeMatch`, authenticated `merge` with merge-redirect `resolve`, `split`, and `mergeHistory`.
...
- Conformance: `/conf/lifecycle/merge`, `/conf/lifecycle/split`, and `/conf/core/reconciliation` pass.

Implements 4D-ID specification 2.4.
